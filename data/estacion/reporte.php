...
if ($isDivisaReport) {
    // Para el reporte con divisa, el total en Bs es: (Divisas * Tasa) + Efectivo Bs + Débito Bs
    $divisaEnBs = ($dataTotal['total_divisa'] ?? 0) * ($dataTotal['tasa_dia'] ?? 0);
    $totalGeneralBs = $divisaEnBs + ($dataTotal['total_efectivo_bs'] ?? 0) + ($dataTotal['total_debito'] ?? 0);
} else {
    // Para el reporte unificado, el total ya viene calculado en 'total_efectivo_bs' y 'total_debito'
        ...
        <table class="table summary-table">
            ...
            ($isDivisaReport ? '
                <tr>
                    ...
                    <th>Divisa ($):</th>
                    <td>' . number_format($dataTotal['total_divisa'] ?? 0, 2) . ' $</td>
                </tr>
                <tr>
                    <th>Tarjeta de Débito (Bs):</th>
                    <td colspan="3">' . number_format($dataTotal['total_debito'] ?? 0, 2) . ' Bs</td>
                </tr>
            ' : '
                <tr><th>Efectivo (Bs):</th><td>' . number_format($dataTotal['total_efectivo_bs'] ?? 0, 2) . ' Bs</td><th>Tarjeta de Débito (Bs):</th><td>' . number_format($dataTotal['total_debito'] ?? 0, 2) . ' Bs</td></tr>
            ') . '
            <tr>
                <th colspan="2">TOTAL GENERAL (Bs):</th>
                <td colspan="2"><strong>' . number_format($totalGeneralBs, 2) . ' Bs</strong></td>
            </tr>
        </table>
    </div>
    ...
    foreach ($dataDetallado as $venta) {
        $html .= '
            <tr>
                ...
                ($isDivisaReport ? '<td class="right">' . number_format($venta['monto_divisa'] ?? 0, 2) . '</td>' : '') .
