...
function inicializarTablaDespachos() {
    tblDespachos = $('#tblDespachos').DataTable({
        ...
        "ajax": {
            // Usar el endpoint específico para órdenes por estado (2 = Aprobadas)
            "url": `${base_url}Orden/getOrdenesPorEstado/2`,
            "dataSrc": "data"
        },
        "columns": [
            { "data": "id_despacho" },
...
async function fntCargarParaDespachar(idDespacho) {
    ...
    try {
        ...
        if (result.success) {
            ...
            const footerForm = btnConfirmar.closest('.card-footer');
            ...
            // Usar el campo correcto para el estado de la requisición
