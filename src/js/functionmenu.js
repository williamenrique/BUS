...
function initializeApp() {
    ...
    $('#select-user').on('change', handleUserSelect); // Usar jQuery para compatibilidad con Select2
...
function populateUserDropdown(users) {
    const select = $('#select-user'); // Usar jQuery para Select2
    select.empty().append('<option value="">-- Seleccione un usuario --</option>'); // Limpiar y añadir opción por defecto

    users.forEach(user => {
        const option = new Option(`${user.usuario_nick} (${user.personal_nombre} ${user.personal_apellido}) - ${user.rol_nombre}`, user.usuario_id);
    ...
    select.select2({
        ...
        theme: 'bootstrap4', // O el tema que estés usando, ej: 'default'
        language: "es"
